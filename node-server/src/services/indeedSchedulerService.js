...
async function saveSchedulerState(orgId, enabled, config = null, lastRunAt = null, lastResult = null, currentKeywordIndex = null, lastKeyword = null) {
  const db = getDB();
...
async function runIndeedTick(orgId) {
  ...
  if (!keywords.length) {
    await saveSchedulerState(orgId, true, null, new Date(), { total_fetched: 0, keyword_ran: null, error: 'No keywords configured' });
    return;
  }

  const currentIndex = (parseInt(state.current_keyword_index) || 0) % keywords.length;
  ...
  // JobSpy is Python-only — Indeed scraping not available in Node.
  // Tick records state progression but does not scrape.
  const lastResult = {
    total_fetched: 0,
    keyword_ran: keyword,
...
export async function startIndeedSchedulerForOrg(orgId, config) {
  ...
  const state = await getSchedulerState(orgId);
  const currentKeywordIndex = parseInt(state?.current_keyword_index) || 0;
