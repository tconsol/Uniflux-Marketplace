...
function buildJobTypeCondition(jobType) {
  if (!jobType) return null;

  const normalized = normalizeJobType(jobType);
  const patterns = JOB_TYPE_MAP[normalized];

  if (patterns?.length) {
    return {
      $or: patterns.map(p => ({
        job_type: { $regex: p, $options: 'i' },
      })),
    };
  }

  const rx = flexibleRegex(jobType);
  if (!rx) return null;
  return { job_type: { $regex: rx, $options: 'i' } };
}
...
function buildQuery({ orgId, keyword, location, jobType, site, skills }) {
  ...
  if (keyword) {
    const rx = flexibleRegex(keyword);
    if (rx) {
      andConditions.push({
        $or: [
  ...
  if (location) {
    const rx = flexibleRegex(location);
    if (rx) {
      query['location.raw'] = { $regex: rx, $options: 'i' };
    }
  }

  if (jobType) {
    const condition = buildJobTypeCondition(jobType);
    if (condition) andConditions.push(condition);
  }

  if (site) {
    query.source_site = String(site).trim().toLowerCase();
  }
...
export async function getPublicJobs({ keyword, location, jobType, site, skills, page = 1, limit = 200 }) {
  ...
  console.log(`[Jobs] getPublicJobs page=${safePage} limit=${safeLimit} skip=${skip} total=${total} query=${JSON.stringify(query)}`);

  const docs = await coll.find(query).sort({ _id: 1 }).skip(skip).limit(safeLimit).toArray();
...
export async function getPublicJobsCount({ keyword, location, jobType, site, skills }) {
  ...
  return {
    ...
    by_job_type: Object.fromEntries(jobTypeAgg.filter(r => r._id).map(r => [normalizeJobType(r._id), r.count])),
  };
}
...
export async function listScrapedJobs({
  ...
  console.log(`[Jobs] listScrapedJobs query=${JSON.stringify(query)} fetchAll=${fetchAll} limit=${safeLimit} skip=${safeSkip}`);

  const total = await coll.countDocuments(query);
  console.log(`[Jobs] matched total=${total}`);

  let cursor = coll.find(query).sort({ scraped_at: -1, _id: -1 });
  if (!fetchAll) {
...
export async function getPublicFilterCounts({
  keyword,
  location,
  jobType,
  site,
  ...
  if (datePosted) {
    ...
    const days = dateMap[datePosted];
    if (days) {
      query.scraped_at = { $gte: new Date(Date.now() - days * 24 * 60 * 60 * 1000) };
    }
  }
  ...
  return Promise.all(
    sites.map(async (site) => {
      const meta = site === 'indeed'
        ? await db.collection('site_fetch_meta').findOne(
            { org_id: orgId, site },
...
export async function getJobCounts({ orgId, keyword, location, isRemote }) {
  const db = getDB();
  const coll = db.collection('scraped_jobs');
  ...
  if (location) {
    const rx = flexibleRegex(location);
    if (rx) {
      baseQuery['location.raw'] = { $regex: rx, $options: 'i' };
    }
  }

  if (isRemote !== undefined) {
    baseQuery['location.is_remote'] = isRemote === 'true';
  }

  const allSites = ['google', 'glassdoor', 'zip_recruiter', 'indeed', 'jsearch', 'linkedin', 'monster', 'dice'];
...
export async function searchAndStoreJsearchJobs(
  {
    keywords,
    location,
    country = 'us',
  ...
  if (useCursor) {
    const result = await jsearchSearchV2({
      query,
      numPages,
      cursor,
      country,
  ...
  await db.collection('site_fetch_meta').updateOne(
    { org_id: orgId, site: 'jsearch' },
    {
      $set: {
        ...
        ...(nextCursor && { last_cursor: nextCursor }),
      },
    },
    { upsert: true }
  );
